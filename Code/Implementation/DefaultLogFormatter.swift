...
public struct DefaultLogFormatter: LogFormatter
{
    ...
    public let colors: [LogSeverity: LogColor]

    public static let defaultColors: [LogSeverity: LogColor] = [
        .Verbose: .lightGrey,
        .Debug: .darkGrey,
        .Info: .blue,
        .Warning: .orange,
    ...
    */
    public init(includeTimestamp: Bool = false, includeThreadID: Bool = false, enableColors: Bool = false, colors: [LogSeverity: LogColor] = defaultColors)
    {
        self.includeTimestamp = includeTimestamp
        self.includeThreadID = includeThreadID
        
        let LogColors = NSProcessInfo.processInfo().environment["XcodeColors"] as? String
        self.enableColors = (LogColors != nil && enableColors)
        self.colors = colors
    }
    ...
    */
        ...
        -> String?
    {
        ...
        let formattedMessage = DefaultLogFormatter.formatLogMessageWithSeverity(severity, caller: caller, message: message, timestamp: timestamp, threadID: threadID)
        
        if enableColors, let LogColor = colors[entry.severity] {
            return LogColor.format(formattedMessage)
        } else {
